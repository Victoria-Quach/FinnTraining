...
public class GAMMEL {
    ...
    public void test() {
        ...
        // 7. Click on Log in (inside shadow dome)
        WebElement LoggInnButton = driver.findElement(By.xpath("//*[@id=\"frontpage-content\"]/finn-topbar")).getShadowRoot().findElement(By.cssSelector("header > nav > a:nth-child(5)"));
        LoggInnButton.click();
        ...
        WebElement reiseButton = wait.until(ExpectedConditions.presenceOfElementLocated(yourElementLocator2));
        reiseButton.click();
        ...
        WebElement direkteButton = driver.findElement(By.xpath("//label[@for='stopoverFilter0']"));
        direkteButton.click();
        ...
        //AVGANG UTREISE
         try {
          ...
          WebElement sliderAvreiseStart = driver.findElement(By.xpath("//div[@aria-label='Utreise tidligst'][@role='slider']"));
          String currentTimeStart = sliderAvreiseStart.getAttribute("aria-valuetext").split(" ")[1];
          sliderAvreiseStart.click();
          ...
          while (!desiredTravelTimeStart.equals(currentTimeStart)){
              ...
              sliderAvreiseStart.sendKeys(Keys.ARROW_RIGHT);
              currentTimeStart = sliderAvreiseStart.getAttribute("aria-valuetext").split(" ")[1];
          }
  
          // Choose tider (end of interval)
          WebElement sliderAvreiseSlutt = driver.findElement(By.xpath("//div[@aria-label='Utreise senest'][@role='slider']"));
          String currentTimeStop = sliderAvreiseSlutt.getAttribute("aria-valuetext").split(" ")[1];
          sliderAvreiseSlutt.click();
          while (!desiredTravelTimeStop.equals(currentTimeStop)){
              sliderAvreiseSlutt.sendKeys(Keys.ARROW_LEFT);
              currentTimeStop = sliderAvreiseSlutt.getAttribute("aria-valuetext").split(" ")[1];
          }
        ...
        WebElement sliderReisetid = driver.findElement(By.xpath("//div[@aria-label='Maks reisetid'][@role='slider']"));
        // Get the current maximum travel time from the slider's aria-valuetext attribute
        String maxTravelTime = sliderReisetid.getAttribute("aria-valuetext");
        ...
        for (int i = 0; i < (TravelTime - desiredTravelTime); i++ ){
            sliderReisetid.sendKeys(Keys.ARROW_LEFT);
        }
        // Check that the slider has the desired value
        String aria_valuetext = sliderReisetid.getAttribute("aria-valuetext");
        ...
        WebElement LavpriskalenderButton = driver.findElement(By.cssSelector("button[data-testid='toggleLowfare']"));

        // Click the "toggleLowfare" button using JavaScript Executor
        ((JavascriptExecutor) driver).executeScript("arguments[0].click();", LavpriskalenderButton);
